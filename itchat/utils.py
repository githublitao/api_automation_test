...
from api_test.models import QRCode, Robot
...
def print_qr(fileDir):
    if config.OS == 'Darwin':
        ...
        QRCode(robot=Robot.objects.get(robotType="WX"), img="/static/img/%sQR.png" % sequence).save()
    elif config.OS == 'Linux':
        # subprocess.call(['xdg-open', fileDir])
        sequence = random_sequence()
        os.system("mv %s %s/frontend/dist/static/img/%sQR.png" % (os.getcwd() + '/' + 'QR.png', os.getcwd(), sequence))
        QRCode(robot=Robot.objects.get(robotType="WX"), img="/static/img/%sQR.png" % sequence).save()
    else:
        sequence = random_sequence()
        os.startfile(fileDir)
        # os.system("move %s %s/frontend/dist/static/img/%sQR.png" % (os.getcwd() + '/' + 'QR.png', os.getcwd(), sequence))
        QRCode(robot=Robot.objects.get(robotType="WX"), img="/static/img/%sQR.png" % sequence).save()
